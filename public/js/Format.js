...
Format.prototype.refresh = function()
{
	...
	else
	{
		...
		this.panels.push(new StyleFormatPanel(this, ui, textPanel));
...
dataForm = function(div, ui, cell)
{
	...
	var addTextArea = function(index, name, value)
	{
		if (name.includes('_'))
		{
			...
			else if (name.includes('_vn'))
			{
				name = name.replace('_vn', '');
				buttonLabel = 'Listen';
			}
			var data = mxUtils.button(buttonLabel, function() {
				...
				if (buttonLabel == 'View')
				{
					...
					ui.showDialog(cont, 480, 420, true, true, null, false);
				}
				else if (buttonLabel == 'Listen')
				{
					...
					inner.style.width = '100%';
					cont.appendChild(inner);
					ui.showDialog(cont, 480, 60, true, true, null, false);
				}
				
			});
			data.id = value;
			var text = form.addField(name + ':', data);
		...
		else
		{
			...
			data.value = value;
			texts[index] = form.addField(name + ':', data);
	...
	top.style.cssText = 'position:relative;overflow-y:auto;';
	top.appendChild(form.table);

	var newProp = document.createElement('div');
	newProp.style.boxSizing = 'border-box';
	newProp.style.paddingRight = '18px';
	newProp.style.whiteSpace = 'nowrap';
	newProp.style.marginTop = '6px';
	newProp.style.marginBottom = '6px';
		...
		if (name.length > 0 && name != 'label' && name != 'placeholders' && name.indexOf(':') < 0)
		{
			try
			{
				...
				else
				{
					...
					names.push(name);
					var text = form.addField(name + ':', document.createElement('input'));
					text.style.width = '100%';
					texts.push(text);
	...
	addBtn.style.paddingLeft = '0px';
	// addBtn.className = 'geBtn';
	linebreak = document.createElement('br');
	newProp.appendChild(linebreak);
	...
	var applyBtn = mxUtils.button(mxResources.get('apply'), function()
	{
		try
		{
			...
			for (var i = 0; i < names.length; i++)
			{
				if (texts[i] == null)
				{
					// if(value.hasAttribute(names[i]))
					value.removeAttribute(names[i]);
					value.removeAttribute(names[i] + '_image');
					value.removeAttribute(names[i] + '_vn');
				}
				else if (texts[i].tagName == 'BUTTON')
				{
					let name = names[i];
					if (texts[i].innerHTML == 'View')
						name += '_image';
					else if (texts[i].innerHTML == 'Listen')
						name += '_vn';
					value.setAttribute(name, texts[i].id);
				}
				else
				{
	...
	buttons.style.cssText = 'margin-left: 2px;margin-right: 16px;'
	...
	upload.style.boxSizing = 'border-box';
	upload.style.paddingRight = '18px';
	upload.style.whiteSpace = 'nowrap';
	upload.style.marginTop = '6px';
	upload.style.marginBottom = '6px';
	upload.style.width = '100%';

	var uploadForm = document.createElement('form');
	uploadForm.id = 'nodeAttachment';
	uploadForm.style.paddingLeft = '2px';

	var uploadLabel = document.createElement('label');
	uploadLabel.innerHTML = 'File Identifier';
	...
	fileUpload.id = 'file';
	fileUpload.style.marginTop = '3px';
	fileUpload.style.marginBottom = '3px';
	...
	imageRadio.style.marginRight = '4px';


	var vnLabel = document.createElement('label');
	vnLabel.innerHTML = 'Voice Note';
	var vnRadio = document.createElement('input');
	vnRadio.type = 'radio';
	vnRadio.name = 'fileType';
	vnRadio.id = 'vn';
	vnRadio.value = 'vn';

	var submit = mxUtils.button('Upload', function()
	{
		...
		xhr.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				...
				else
				{
					...
					var buttonLabel = '';
					if (formData.get('fileType') == 'image')
						buttonLabel = 'View';
					else
						buttonLabel = 'Listen';
					var data = mxUtils.button(buttonLabel, function() {
				...
				document.getElementById('nodeAttachment').reset();
				
				console.log(xhr.responseText); 
			}
		}
		xhr.open("POST", '/uploadMedia');    
		xhr.send(formData);
	});
	submit.style.textOverflow = 'ellipsis';
	submit.style.overflow = 'hidden';
	submit.style.width = '100%';
	submit.style.marginLeft = '2px';
	submit.style.marginTop = '3px';
	submit.style.paddingLeft = '0px';

	uploadForm.appendChild(uploadLabel);
	uploadForm.appendChild(fileName);
	uploadForm.appendChild(document.createElement('br'));
	uploadForm.appendChild(fileUpload);
	uploadForm.appendChild(document.createElement('br'));
	uploadForm.appendChild(imageLabel);
	uploadForm.appendChild(imageRadio);
	uploadForm.appendChild(vnLabel);
	uploadForm.appendChild(vnRadio);
	upload.appendChild(uploadForm);
...
TextFormatPanel.prototype.addFont = function(container)
{
	...
	var ss = this.format.getSelectionState();
	// EASY ELICITATION
	if(ui.editor.graph.getModel().isVertex(ss.vertices[0]))
	{
		var dataTitle = this.createTitle("Data");//this.createPanel();
		dataTitle.style.paddingLeft = '18px';
		dataTitle.style.paddingTop = '10px';
		dataTitle.style.paddingBottom = '6px';
		container.appendChild(dataTitle);

		var dataPanel = this.createPanel();
		dataPanel.style.paddingTop = '2px';
		dataPanel.style.paddingBottom = '2px';
		dataPanel.style.position = 'relative';
		dataPanel.style.marginLeft = '-2px';
		dataPanel.style.borderWidth = '0px';
		dataPanel.style.paddingBottom = '10px';
		dataPanel.style.borderBottom = '1px solid rgb(192, 192, 192)';
		// dataPanel.style.height = '200px';
		dataPanel.className = 'geToolbarContainer';
		dataPanel = dataForm(dataPanel, ui, ss.vertices[0]);
